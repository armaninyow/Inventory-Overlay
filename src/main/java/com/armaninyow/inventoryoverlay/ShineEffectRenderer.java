...
public class ShineEffectRenderer {
	...
	public void renderShine(DrawContext context, int slotIndex, int slotX, int slotY) {
		...
		// At progress 0: line is outside top-left corner
		// At progress 1: line is outside bottom-right corner
		
		// Total distance the line travels (diagonal of the slot + padding on both sides)
		...
		// We'll draw pixels where x + y = currentPosition (approximately)
		for (int y = 0; y < SLOT_SIZE; y++) {
			for (int x = 0; x < SLOT_SIZE; x++) {
				// Calculate this pixel's position along the diagonal
				float diagonalPos = x + y;
				
				// Check if this pixel is within the line width
